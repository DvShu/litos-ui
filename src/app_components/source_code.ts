...
export default class SourceCode extends HTMLElement {
  async connectedCallback() {
    let $textareas = elem("textarea", this) as HTMLTextAreaElement[];
    if ($textareas == null || $textareas.length === 0) {
      const $template = elem(".template", this)[0];
      if ($template != null) {
        $textareas = elem("textarea", $template) as HTMLTextAreaElement[];
      }
    }
  ...
  async tagCode(area: HTMLTextAreaElement, active = false) {
  ...
  async parseCodeTempte(preCode: string, lang = "ts") {
